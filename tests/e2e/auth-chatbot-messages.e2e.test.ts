...
describe("e2E auth, chatbot, contacts, conversations, messages", () => {
 ...
 it("get contacts and verify test@example.com belongs to current user", async () => {
  ...
  const contactsBody = await readJson<
   Array<{
    id: number;
    email: string;
    userId: number;
 ...
 it("send messages using all query_test data for proposed chatbot", async () => {
  ...
  for (const item of testQueries) {
   ...
   expect(messageBody.results.debug_info).toBeDefined();

   const debugInfo = messageBody.results.debug_info as Record<string, unknown>;
   expect(debugInfo.complexity_analysis).toBeDefined();
   expect(Array.isArray(debugInfo.mmr_iterations)).toBe(true);
  }
 ...
 it("send messages using all query_test data for baseline chatbot", async () => {
  ...
  for (const item of testQueries) {
   ...
   const debugInfo = messageBody.results.debug_info as Record<string, unknown>;
   expect(Array.isArray(debugInfo.top5_similarities)).toBe(true);
   expect(Array.isArray(debugInfo.top5_document_indices)).toBe(true);
  }
