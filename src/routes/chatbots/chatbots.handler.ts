...
export const getOne: AppRouteHandler<GetChatbot> = async (c) => {
 ...
 // ── Public access (widget) — no auth required ──────────────
 if (!userId) {
  const publicChatbot = await db.query.chatbots.findFirst({
   where(fields, operators) {
 ...
 const testContact = await db.query.contacts.findFirst({
  where(fields, operators) {
   return operators.and(operators.eq(fields.userId, userId), operators.eq(fields.email, "test@example.com"));
  },
 });
 ...
 if (testContact) {
  testConversation = await db.query.conversations.findFirst({
   where(fields, operators) {
    return operators.and(operators.eq(fields.chatbotId, id), operators.eq(fields.contactId, testContact.id), operators.eq(fields.userId, userId));
   },
  });
 }
 ...
 const [conversationsResult, messagesResult, aiResponsesResult] = await Promise.all([
  ...
  db.select({ count: count() }).from(conversations).where(eq(conversations.chatbotId, id)),

  // Count total messages for this chatbot (through conversations)
  db.select({ count: count() }).from(messages).innerJoin(conversations, eq(messages.conversationId, conversations.id)).where(eq(conversations.chatbotId, id)),
...
export const create: AppRouteHandler<CreateChatbot> = async (c) => {
 ...
 if (!pdf || !(pdf instanceof File)) {
  return c.json({ message: "PDF file is required" }, HttpStatusCodes.BAD_REQUEST);
 }

 // validate pdf size max 10MB
 if (pdf.size > 10 * 1024 * 1024) {
  return c.json({ message: "PDF file size exceeds 10MB" }, HttpStatusCodes.BAD_REQUEST);
 }
 ...
 try {
  ...
  if (chatbotData.embeddingModel === "pinecone") {
   ...
   const trainingResponse = await sendTrainingRequestWithoutRetry(form, env.API_PASSWORD, traningType);
 ...
 try {
  ...
  if (pdf && pdf instanceof File) {
   ...
   if (embeddingModel === "pinecone") {
    ...
    const isProposedModel = cleanUpdates.isProposedModel !== undefined ? cleanUpdates.isProposedModel : existingChatbot.isProposedModel;

    const trainingType: "baseline-model" | "proposed-model" = isProposedModel ? "proposed-model" : "baseline-model";

    const trainingResponse = await sendTrainingRequestWithoutRetry(form, env.API_PASSWORD, trainingType);
  ...
  const [updatedChatbot] = await db.update(chatbots).set(cleanUpdates).where(eq(chatbots.id, id)).returning();

  if (!updatedChatbot) {
   return c.json({ message: HttpStatusPhrases.INTERNAL_SERVER_ERROR }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
  }

  return c.json({ message: "Chatbot updated successfully" }, HttpStatusCodes.OK);
 } catch (error) {
...
export const remove: AppRouteHandler<DeleteChatbot> = async (c) => {
 ...
 return c.json({ message: "Chatbot deleted successfully" }, HttpStatusCodes.OK);
};
